...
public class BLecDBOperation {
    ...
    public boolean addlecturer(BLecturer bl) {
        try {

            System.out.println("Add lecturer 1");
            con = (Connection) DriverManager.getConnection(url, username, password);
            String query = "INSERT INTO schoolofbusinesslecturers VALUES (?,?,?,?,?,?,?,?,?,?,?,?)";
    ...
    ArrayList<BLecturer> getlecturer() {
        try {

            ArrayList<BLecturer> list = new ArrayList<BLecturer>();

            System.out.println("Get lecturer 1");
            con = (Connection) DriverManager.getConnection(url, username, password);
            String query = "SELECT * FROM schoolofbusinesslecturers";
            pst = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(query);
    ...
    public boolean deletelecturer(BLecturer bl) {
        try {
            
            System.out.println("Delete lecturer 1");
            con = (Connection) DriverManager.getConnection(url, username, password);
            String query = "DELETE FROM schoolofbusinesslecturers WHERE lecid="+ bl.getLecid();//lecid=" + bl.getLecid()
            pst = (com.mysql.jdbc.PreparedStatement) con.prepareStatement(query);
